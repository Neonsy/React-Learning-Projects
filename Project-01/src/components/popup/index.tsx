...
export default function Popup({ children }: Props) {
    ...
    return (
        showPopup && (
            <div id='popup-overlay' ref={overlayRef} onClick={(e) => closeViaOverlay(e)} className='fixed top-0 left-0 w-full h-full bg-black/50'>
                <div
                    id='popup-content'
                    ...
                    {children}
                    <button onClick={() => setShowPopup(false)} className='absolute top-1 right-1.5 self-end text-2xl text-blue-500 hover:underline'>
                        <IoCloseSharp />
                    </button>
                </div>
            </div>
